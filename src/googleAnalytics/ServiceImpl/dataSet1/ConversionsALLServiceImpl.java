...
//Yearly Data
@Service("conversionsALLService")
public class ConversionsALLServiceImpl implements DataSetService {
	static int count = 0;
	...
	public List<Report> fetchData(AnalyticsReporting service, String viewId) throws IOException {
		...
		ReportRequest request = new ReportRequest().setViewId("ga:" + viewId).setDateRanges(Utility.yearlyDateRange())
				.setDimensions(dimensionsList).setMetrics(Arrays.asList(goalCompletions)).setIncludeEmptyRows(true)
				.setPageSize(5000);
		...
		for (Report report : reports) {
			...
			for (ReportRow row : rows) {
				...
				if (Integer.valueOf(metrics.get(0).getValues().get(0)) == 0 && flag == 0) {
					...
					count++;
					flag = 1;
					ConversionsALL bean = new ConversionsALL();
					...
					writeDataToCassandra.writeConversionsAll(bean);
				}
			}
		}
	}
	...
	public int countTableRows() {
		...
		return ConversionsALLServiceImpl.count;
	}
