...
//Monthly Data
@Service("convOverviewPPCService")
public class ConvOverviewPPCServiceImpl implements DataSetService {
	static int count = 0;
	...
	public List<Report> fetchData(AnalyticsReporting service, String viewId) throws IOException {
		...
		ReportRequest request = new ReportRequest().setViewId("ga:" + viewId).setDateRanges(Utility.monthlyDateRange())
				.setDimensions(dimensionsList).setSegments(Arrays.asList(mediumSegment)).setMetrics((metricList))
				.setIncludeEmptyRows(true).setPageSize(10000);
				...
				for (ReportRow row : rows) {
					try {
						...
						if (Integer.valueOf(metrics.get(0).getValues().get(0)) == 0 && flag == 0) {
							...
							count++;
							flag = 1;
							ConvOverviewPPC bean = new ConvOverviewPPC();
							...
							writeDataToCassandra.writeConvOverviewPPC(bean);
						}
	...
	public int countTableRows() {
		...
		return ConvOverviewPPCServiceImpl.count;
	}
