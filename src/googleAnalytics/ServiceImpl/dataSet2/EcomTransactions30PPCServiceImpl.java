...
//Monthly Data
@Service("ecomTransactions30PPCService")
public class EcomTransactions30PPCServiceImpl implements DataSetService {
	static int count =0;
	@Override
	public List<Report> fetchData(AnalyticsReporting service, String viewId) throws IOException {
		...
		Segment segment = Utility.buildSimpleSegment("Session with Medium", "ga:medium", "EXACT", "cpc");
		...
		for (Report report : reports) {
			...
			for (ReportRow row : rows) {
				...
				if (Integer.valueOf(metrics.get(0).getValues().get(0)) == 0 && flag == 0) {
					...
					count++;
					flag = 1;
					EcomTransactions30PPC bean = new EcomTransactions30PPC();
...
bean.setEndDate(String.valueOf(format.format(yesterday.getTime())));
					writeDataToCassandra.writeEcomTransactions30PPC(bean);
				}
			}
		}
	}
	...
	public int countTableRows() {
		...
		return EcomTransactions30PPCServiceImpl.count;
	}
