...
public class EcomTransactions30directServiceImpl implements DataSetService {
	static int count =0;
	@Override
	public List<Report> fetchData(AnalyticsReporting service, String viewId) throws IOException {
		...
		Segment segment = Utility.buildSimpleSegment("Session with Medium", "ga:medium", "EXACT", "(none)");
		...
		for (Report report : reports) {
			...
			for (ReportRow row : rows) {
				...
				if (Integer.valueOf(metrics.get(0).getValues().get(0)) == 0 && flag == 0) {
					...
					count++;
					flag = 1;
					EcomTransactions30direct bean = new EcomTransactions30direct();
					...
					writeDataToCassandra.writeEcomTransactions30direct(bean);
				}
			}
		}
	}
	...
	public int countTableRows() {
		...
		return EcomTransactions30directServiceImpl.count;
	}
