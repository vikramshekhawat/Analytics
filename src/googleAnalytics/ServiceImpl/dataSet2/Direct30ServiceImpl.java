...
//Monthly Data
@Service("direct30Service")
public class Direct30ServiceImpl implements DataSetService {
	static int count = 0;
	...
	public List<Report> fetchData(AnalyticsReporting service, String viewId) throws IOException {
		...
		Segment mediumSegment = Utility.buildSimpleSegment("Sessions with Medium", "ga:medium", "EXACT", "(none)");
			...
			if (rows != null) {
				for (ReportRow row : rows) {
					...
					if (Double.valueOf(metrics.get(0).getValues().get(0)) == 0 && flag == 0) {
						...
						writeDataToCassandra.writeDirect30Data(bean);
					}
				}
			}
	...
	public int countTableRows() {
		...
		return Direct30ServiceImpl.count;
	}
