...
@Service("goalConversion4Service")
public class GoalCompletion4ServiceImpl implements DataSetService {
	static int count = 0;
	...
	public void saveData(List<Report> reports, String client_stamp) { 
		...
		List<String> list = obj.getGoal(4);
		for (Report report : reports) {
			...
			for (ReportRow row : rows) {
				...
				if (Integer.valueOf(metrics.get(0).getValues().get(0)) == 0 && flag == 0 && rows.size() > 10) {
					continue;
				} else {
					count++;
					flag = 1;

					GoalCompletion4 bean = new GoalCompletion4();
					...
					writeDataToCassandra.writeGoalConversion4(bean);

				}
			}
		}
		latestTableObj.saveData(env.getProperty("27"), client_stamp, Utility.currentDate().get("dayOfYear"),
	...
	public int countTableRows() {
		...
		return GoalCompletion4ServiceImpl.count;
	}
